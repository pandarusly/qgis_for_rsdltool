    ...
    def __init__(self, features, out_features=None, dilations=(1, 2, 4, 8), act_cfg=None, **kwargs):
        ...
        self.append(
            nn.Sequential(
                ...
                nn.Identity() if act_cfg is None else nn.ReLU()
            )
        )
...
class VVITv3(nn.Module):
    def __init__(self, image_size, dim, patch_size=1, num_frames=2, depth=2, heads=3, in_channels=3, dim_head=64, sr_ratio=2,
        ...
        self.space_transformer = STransformer(
            dim=dim, depth=depth, heads=heads, dim_head=dim_head, mlp_dim=dim * scale_dim, sr_ratio=sr_ratio, dropout=dropout, h=self.resolution, w=self.resolution)
    ...
    def __init__(self, filters, kernel_size: int = 3, norm_cfg=None, conv_cfg=None,
        ...
        for i in range(1, len(filters)):
            layers.extend([
                ...
                nn.ReLU() if act_cfg else nn.Identity()
            ])
...
class SimpleHead(BaseDecodeHead, ABC):
    ...
    def forward_single(self, inputs: torch.Tensor):
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.attention2 = nn.Identity()  # create_attn(attention_type, out_channels)
...
class UnetHeadTransMuti(SimpleHead):
    def __init__(
        ...
        self.predicts = nn.ModuleList([ResidualCls(uout_channels[0], self.num_classes), ResidualCls(
            uout_channels[1], self.num_classes), ResidualCls(uout_channels[2], self.num_classes), ResidualCls(uout_channels[3], self.num_classes)])
...
class UVASPSingle(nn.Module):
    def __init__(self,
        ...
        self.backbone = resnet50(**encoder_set)
        # 创建 FPN增强--------------------------------
        self.with_neck = neck["with_neck"]
        self.neck = FPN([256, 512, 1024, 2048], neck["neck_out"],
                        len([256, 512, 1024, 2048]))
        # --------------------------------

        fea1_nums = [32, 32, 32, 32]  # [64, 128, 256, 512]

        self.decoder = UnetHeadTransSingle(
            in_channels=fea1_nums, **decoder_set)
...
class UVASPMuti(nn.Module):
    def __init__(self,   pretrained=None,
        ...
        super().__init__()
        from timm.models import resnet50d, resnet50
        ...
        # --------------------------------
        fea1_nums = [neck['neck_out'], neck['neck_out'],
                     neck['neck_out'], neck['neck_out']]  # [64, 128, 256, 512]
    ...
    def init_weights(self):
        init_weights(self.neck) if self.neck is not None else None
        init_weights(self.decoder)
        if self.pretrained is not None:
            print('use pretrain in {}'.format(self.pretrained))
            pretained_dict = torch.load(self.pretrained)
            self.backbone.load_state_dict(pretained_dict)
        else:
...
if __name__ == "__main__":
    moddl = UVASPMuti(
        pretrained=r'/workspace/data/pretrained/resnet50_a1_0-14fe96d1.pth')
