...
def build_feat_extractor(in_ch, width, arch='resnet18', pretrained=True):
    return nn.Sequential(
        Backbone(in_ch, arch,pretrained=pretrained),
        Decoder(width)
    )
    ...
    def __init__(self, in_ch, arch, pretrained=True, strides=(2, 1, 2, 2, 2)):
        ...
        if arch == 'resnet18':
            self.resnet = resnet.resnet18(
                pretrained=pretrained, strides=strides, norm_layer=get_norm_layer())
        elif arch == 'resnet34':
...
class Decoder(nn.Module, KaimingInitMixin):
    ...
    def forward(self, feats):
        ...
        f2 = F.interpolate(
            f2, size=f1.shape[2:], mode='bilinear', align_corners=True)
        f3 = F.interpolate(
            f3, size=f1.shape[2:], mode='bilinear', align_corners=True)
        f4 = F.interpolate(
            f4, size=f1.shape[2:], mode='bilinear', align_corners=True)
...
class PAMBlock(nn.Module):
    ...
    def _split_subregions(self, x):
        ...
        x = x.reshape((b, c, self.scale, h // self.scale,
                      self.scale, w // self.scale))
        x = x.permute((0, 2, 4, 1, 3, 5)).reshape(
            (b * self.scale * self.scale, c, -1))
        return x

    def _recons_whole(self, x, b, c, h, w):
        x = x.reshape((b, self.scale, self.scale, c,
...
class STANet(nn.Module):
    def __init__(self, in_ch, width=64, att_type='BAM', ds=1,arch='resnet18', pretrained=True):
        super().__init__()
        self.pretrained = pretrained
        self.extract = build_feat_extractor(in_ch=in_ch, width=width,pretrained=pretrained,arch=arch)
        self.attend = build_sta_module(in_ch=width, att_type=att_type, ds=ds)
        self.calc_dist = nn.PairwiseDistance(keepdim=True)

    def init_weights(self):
        if self.pretrained:
            init_weights(self.attend)
            init_weights(self.calc_dist)
        else:
            init_weights(self)
    ...
    def forward(self, t1, t2):
        ...
        dist = F.interpolate(
            dist, size=t1.shape[2:], mode='bilinear', align_corners=True)
        return dist
...
if __name__ == '__main__':
    ...
    model = model.cuda()
    output = model(toy_data1, toy_data2)
