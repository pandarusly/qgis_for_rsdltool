...
class ChangeEncoderDecoder(EncoderDecoder):
    ...
    def __init__(self, change, **kwargs):
        super(ChangeEncoderDecoder, self).__init__(**kwargs)

        self.change = builder.build_change(change)
        assert self.with_change
    ...
    def forward(self, img1, img2):
        ...
        if self.with_auxiliary_head:
            ...
            if isinstance(self.auxiliary_head, nn.ModuleList):
                ...
                result.append(self.auxiliary_head(x))
            return result
        return seg_logits
    ...
    def forward_train(self, img1, img2, img_metas, gt_semantic_seg):
        ...
        if self.with_auxiliary_head:
            loss_aux, _ = self._auxiliary_head_forward_train(
                x, img_metas, gt_semantic_seg
            )
            losses.update(loss_aux)
