...
import math
# ----------加载自己模型
# from src.UVALitModelSingle import UVALitModelSingle, UVALitModelMuti
from itertools import repeat
from typing import List, Iterable, Tuple, Any, Union, Sequence
...
def get_path_from_yaml(path):
    ...
    if exist:
        paths = OmegaConf.to_object(OmegaConf.load(path))
        return paths
    else:
        return None
...
class ImageSlicer:
    ...
    def __init__(self, image_shape, tile_size, tile_step=0, image_margin=0, weight="mean"):
        ...
        if isinstance(tile_size, (np.ndarray, Sequence)):
            if len(tile_size) != 2:
                raise ValueError(
                    f"Tile size must have exactly 2 elements. Got: tile_size={tile_size}")
        ...
        if isinstance(tile_step, (np.ndarray, Sequence)):
            if len(tile_step) != 2:
                raise ValueError(
                    f"Tile size must have exactly 2 elements. Got: tile_step={tile_size}")
        ...
        self.weight = weight if isinstance(
            weight, np.ndarray) else weights[weight](self.tile_size)
        ...
        overlap = (self.tile_size[0] - self.tile_step[0],
        ...
        if image_margin == 0:
            ...
            nw = max(1, math.ceil(
                (self.image_width - overlap[1]) / self.tile_step[1]))
            nh = max(1, math.ceil(
                (self.image_height - overlap[0]) / self.tile_step[0]))
        ...
        for y in range(
                0, self.image_height + self.margin_top + self.margin_bottom -
                   self.tile_size[0] + 1, self.tile_step[0]
        ):
            for x in range(
                    0, self.image_width + self.margin_left +
                       self.margin_right - self.tile_size[1] + 1,
                    self.tile_step[1]
            ):
                crops.append((x, y, self.tile_size[1], self.tile_size[0]))
                bbox_crops.append(
                    (x - self.margin_left, y - self.margin_top, self.tile_size[1], self.tile_size[0]))
    ...
    def merge(self, tiles: List[np.ndarray], dtype=np.float32):
        ...
        norm_mask = np.clip(norm_mask, a_min=np.finfo(
            norm_mask.dtype).eps, a_max=None)
...
class TileMerger:
    ...
    def __init__(self, image_shape, channels, weight, device="cpu", dtype=torch.float32):
        ...
        self.weight = torch.from_numpy(np.expand_dims(
            weight, axis=0)).to(device=device, dtype=dtype)
        self.image = torch.zeros(
            (channels, self.image_height, self.image_width), device=device, dtype=dtype)
        self.norm_mask = torch.zeros(
            (1, self.image_height, self.image_width), device=device, dtype=dtype)
    ...
    def integrate_batch(self, batch: torch.Tensor, crop_coords):
        ...
        for tile, (x, y, tile_width, tile_height) in zip(batch, crop_coords):
            self.image[:, y: y + tile_height, x: x +
                                                 tile_width] += tile * self.weight
            self.norm_mask[:, y: y + tile_height,
            x: x + tile_width] += self.weight
