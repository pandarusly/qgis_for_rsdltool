...
def ImageRowCol2Projection(adfGeoTransform, iCol, iRow):
    ...
    dProjX = adfGeoTransform[0] + adfGeoTransform[1] * \
        iCol + adfGeoTransform[2] * iRow
    dProjY = adfGeoTransform[3] + adfGeoTransform[4] * \
        iCol + adfGeoTransform[5] * iRow
...
def GetGeoExtent(x_off, y_off, cropsize, adfGeoTransform):
    ...
    endX, endY = ImageRowCol2Projection(
        adfGeoTransform, x_off + cropsize, y_off + cropsize)
...
def is_label_useful(array, threshold=500, len_contours=0, **kwargs) -> bool:
    if threshold == 0:
        return True
    _, thresh = cv2.threshold(array, thresh=1.5, maxval=255,
                              type=cv2.THRESH_BINARY + cv2.THRESH_OTSU)  # 统一变成255前景
    contours, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 寻找连通域
    for i, cnt in enumerate(contours):
        area = cv2.contourArea(contours[i])
        if area < threshold:
            cv2.drawContours(thresh, [contours[i]], -1, 0, -1)
    contours, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # 填充后寻找连通域
...
def ExportTile(ds, cdpath, file_name, startX, startY, x_off, y_off, gt, cropsize, imgfrmat, frmt_ext, i, j,
    ...
    scanline = ds.ReadRaster(x_off, y_off, cropsize,
                             cropsize, cropsize, cropsize, gdal.GDT_Byte)
    mem_ds.WriteRaster(0, 0, cropsize, cropsize, scanline,
                       cropsize, cropsize, gdal.GDT_Byte)
    array = mem_ds.GetRasterBand(1).ReadAsArray()
    width, height = mem_ds.GetRasterBand(
        1).XSize, mem_ds.GetRasterBand(1).YSize
    ...
    if imgfrmat != "JPEG":
        ...
        array = array * \
            255 if np.max(array) < 255 and len(
                np.unique(array)) == 2 else array
        mem_ds.GetRasterBand(1).WriteArray(array)

    drv = gdal.GetDriverByName(imgfrmat)
    imagepath = osp.join(cdpath, (str(file_name) + "-" +
                         str(j) + "-" + str(i) + "." + frmt_ext))
...
# Start Splitting
def splitting(fn_ras, cdpath, frmt_ext, imgfrmat, scaleoptions, cropsize, stridesize, file_name,
              youhua=dict(valpixel=0.2, threshold=500, len_contours=0)):
    ds = gdal.Open(fn_ras)
    ...
    for i in range(rows_max + 1):
        for j in range(columns_max + 1):
            ...
            startX, endX, endY, startY = GetGeoExtent(
                x_off, y_off, cropsize, gt)
